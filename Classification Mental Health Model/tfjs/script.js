document.addEventListener('DOMContentLoaded', function() {
    ...
    // load sample data - SAMPLE 3
    document.getElementById('load-data-btn').addEventListener('click', function () {
        // Personal Details dari index ke-2
        document.getElementById('age').value = '18-22';
        document.getElementById('gender').value = 'Female';
        document.getElementById('academic_year').value = 'First Year or Equivalent';
        document.getElementById('cgpa').value = '3.40 - 3.79';
        document.getElementById('got_scholarship').value = 'No';

        // Load assessment data dari index ke-2
        const assessmentData = {
            anxiety: [0, 2, 0, 0, 1, 0, 0],
            stress: [1, 2, 1, 1, 4, 2, 4, 3, 1, 1],
            depression: [2, 0, 0, 0, 0, 1, 0, 0, 0]
        };
    ...
    function displayResults(results) {
        ...
        for (const [condition, data] of Object.entries(results)) {
            ...
            badge.style.backgroundColor = config.labelColor;
            ...
            // Sort probabilities based on severity order
            const sortedProbs = config.severityOrder.map(severityLabel => {
                const found = data.probabilities.find(p => p.label === severityLabel);
                return found || { label: severityLabel, probability: 0 };
            });
            ...
            for (const p of sortedProbs) {
                const row = list.insertRow();
                const labelCell = row.insertCell();
                const valueCell = row.insertCell();

                labelCell.textContent = p.label;
                valueCell.textContent = `${(p.probability * 100).toFixed(1)}%`;
